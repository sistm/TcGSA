...
TcGSA (Time-course Gene Set Analysis *Hejblum et al., 2015*) package test the gene expression among the gene set and over the time. The gene set are made from the common biological utility of genes. TcGSA is based on the linear mixed model to take into account the potential heterogeneity of expression within the gene sets. For more details, check the article [here](http://dx.plos.org/10.1371/journal.pcbi.1004310).

# Getting started using TcGSA

3 inputs are required to run `TcGSA` function :
...
A gene set is a group of gene with the same biological function. It works well to detect difference gene expression and seems to be more powerful than a gene-by-gene analysis. Several definitions of groups have been made, among the most important :
...
2 methods to build the gene set object, the easy one and the hard one.
...
This file contains the 3 kind of gene set detailed above. But carefull with the version of the gene set database because they can be outdated.

To make sure to have the lastest version of database, you should create the `gmt` gene set object from the hard method below.
...
* Column 1 : the name of the gene set
* Column 2 : the description of the gene set
* Remaining columns : the list of gene included in the gene set (representing by the probes)
...
This matrix contains the gene expression (in cells) for each gene (in rows) of each sample (in columns) gathered by microarray analysis. The gene expression should be already normalized before using `TcGSA`. In rows, the names of each gene match to the names of probes (representing genes) in the `gmt` gene set object. 
...
* Time measurements
* _In case of multiples treatments_, the name of treatment
...
This example comes from *Hejblum et al., 2015* and DALIA-1 HIV therapeutic vaccine trial. The aim of this study is to evaluate the immune response to HIV vaccine. To conduct this study, 19 patients contaminated by the HIV have been followed during 48 weeks splited into 2 phases of 24 each :
...
* 2nd phase from week 24 to week 48 : for the follow-up of patients, none of trial vaccine has been injected and antiretroviral treatments have been interruped (except for health problem).

Blood samples have been collected on the different time measurement for each patient to detect the variability of gene expression over the time. For more details, check the article [here](http://dx.plos.org/10.1371/journal.pcbi.1004310).
...
The design data matrix (called `design_preATI`) is built from one of the GEO supplementary files. It contains the required variables plus some additional for this study. Some reprocessing are made according with the source of this paper.
...
* `Patient_ID` for the identifiant of patients
...
This function provides the result of likelihood ratio test using the linear mixte model for each gene set. For this example, we use gene sets data from Chaussabel's modules (*Chaussabel et al., 2008*). `TcGSA.LR` function requires :
...
Now `tcgsa_result` is your TcGSA object containing the form of time trend (defaut option is `linear`), the number of treatment groups and the number of gene sets tested depending on which gene set base is defined. To get the number of significant gene set, you can also use `summary` function on your TcGSA object.

```{r summary_dalia, cache=TRUE}
summary(tcgsa_result)
```

To get more details on the significant gene sets, use `signifLRT.TcGSA` function. It returns several informations such as the significant gene sets amoung all the gene sets tested and their p-values with adjustement for multiple testing (defaut option `BY` for Benjamini-Yekutieli correction *Benjamini et Yekutieli, 2001*) and 5% threshold. Here, an example of five significant gene sets.

```{r signifLRT_ST, cache=TRUE}
TcGSA::signifLRT.TcGSA(tcgsa_result)$mixedLRTadjRes[1:5,]
```

You can also use `multtest.TcGSA` function to provide the likelihood ratios, the raw and ajusted p-values for the whole gene sets with 5% threshold. Here, an example of five results :

```{r multtest_ST, cache=TRUE}
TcGSA::multtest.TcGSA(tcgsa_result)[1:5,]
```

`CVG_H0` and `CVG_H1` are the convergence of the model under null and alternative hypothesis. Close to 0 means a good convergence of the model.

## Graphical output

`plot1GS` plots the different representations of the gene expression in a specific gene set defined by `geneset.name` argument. `plot1GS` function requires :

* `expr` : either the name of the gene expression matrix or the estimations from linear mixte model. In this example, we used the raw data from gene expression matrix.
* `gmt` : name of the gmt gene set object
...
* `time_unit` __OPTIONAL__ : value be displayed next to the values of `TimePoint` on the x-axis
...
Dotted line shows the trend(s) of the gene set over the time.

# How to use TcGSA for several treatment group ?

This example comes from *Obermoser et al. 2013* to study the responses to influenza and pneumococcal vaccines on healthy patients using the gene expression over the time.
The patients are splited into 3 groups of 6, receiving either 2009-2010 seasonal influenza vaccine (Fluzone), a 23-valent pneumococcal vaccine (Pneumovax23) or a placebo.  
Blood samples have been taken on day -7, 0, 1, 3, 7, 10, 14, 21 and 28 to analyse gene expression over time. For more details, check the article [here](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3492754/).
...
In this case, the design and expression data matrix are not available on the GEO website using GEO access number. We have to dowload the raw file and import with `getGEO` function.
...
In opposite to the first example, more processing should be done on these data to built the design and gene expression data.
...
* `subject_name` : name of the identification of patients in the design data matrix
* `time_name` : name of the time measurements in the design data matrix
* `group_name` : name of the group of treatment in the design data matrix
...
`clust` shows the number of trend within the significant gene sets.

### Plot heatmap

`plot` draws differents kind of graphics, but we focus on heatmap graphics. This function requires :
...
`getGEOSuppFiles` provides several files including the gene expression file and design data file and don't need to reprocessing them. Sometimes, theses files are not available on GEO website. In this case, you have to run the `getGEO` function from file containing the raw data. Moreover, you need some reprocessing to built the gene expression, design data matrix. An example for each function is available on "How to use TcGSA for one treatment group ?" and "How to use TcGSA for several treatment group ?" parts.
