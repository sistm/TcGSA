	...
	function(design, subject_name="Patient_ID", time_name="TimePoint", 
		...
		if (!crossedRandom){
			if(group_name=="" & !separateSubjects){
				...
				formula_H1 = paste("expression ~ 1", covariates_fixed, " + ", time, time_covariates,
								   " + (1 +", time, "|probe) + (1|", subject_name, ")", sep="")
				...
				formula_H0 = paste("expression ~ 1", covariates_fixed,
								   " + (1|", subject_name, ") + ", time, time_covariates, " + (1 + ", time, "|probe)",sep="")
				
				formula_H1 = paste("expression ~ 1", covariates_fixed, " + ", time, time_covariates,
								   " + (1 +", time, "|probe) + (1 +", time, "|", subject_name, ")", sep="")
				
				formula_H0_1probe = paste("expression ~ 1", covariates_fixed,
										  " + (1|", subject_name, ") + ", time, time_covariates, sep="")
				
				formula_H1_1probe = paste("expression ~ 1", covariates_fixed, " + ", time, time_covariates,
										  " + (1 +", time, "|", subject_name, ")", sep="")
				
			}else if(group_name!="" & !separateSubjects){
				formula_H0 = paste("expression ~ 1 + ", group_name, covariates_fixed, " + ", time, time_covariates,
								   " + (1 + ", time, "|probe) + (1|", subject_name, ")", sep="")
				
				formula_H1 = paste("expression ~ 1 + ", group_name, covariates_fixed, " + ", time, " + ", paste("(", time, "):", group_name, sep=""), time_covariates,
			...
			if(group_name=="" & !separateSubjects){
				...
				formula_H1 = paste("expression ~ 1 + probe", covariates_fixed,
								   " + ", time, time_covariates, " + " ,time, ":probe + (1 + ", time, "|", subject_name,":probe)", sep="")
				...
				if(time_func=="cubic"){
					...
					formula_H1 = paste("expression ~ 1 + probe", covariates_fixed,
									   " + ", time, time_covariates, " + " ,strsplit(time,"+ ")[[1]][1], ":probe + ",
									   strsplit(time,"+ ")[[1]][3], ":probe + ", strsplit(time,"+ ")[[1]][5], ":probe + (1 + ", time, "|", subject_name,":probe)", sep="")					
				}
