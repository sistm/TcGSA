...
MYheatmap.2 <- function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
						 ...
						 notecol = "cyan", na.color = graphics::par("bg"), trace = c("column", 
						 															"row", "both", "none"), tracecol = "cyan", hline = stats::median(breaks), 
						 vline = stats::median(breaks), linecol = tracecol, margins = c(5, 
						 ...
						 ...) 
{
	...
	if (!missing(rowsep)) 
		for (rsep in rowsep) graphics::rect(xleft = 0, ybottom = (ncol(x) + 
