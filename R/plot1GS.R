...
plot1GS <-
	function(expr, gmt, Subject_ID,TimePoint, geneset.name,
		...
		if(is.data.frame(expr)){
			...
			if(is.null(dim(expr[[interest]]))){
				stop("No information of gene expression for this geneset")
			}else{
# 				if(expr$fit$CVG_H0[[interest]]$opt==0){
# 					cat("The model set up for the gene set", gmt$geneset.names[[interest]] ,"have converged\n")
# 				}else{
# 					cat("The model set up for the gene set", gmt$geneset.names[[interest]] ,"doesn't converged\n")
# 				}
		...
		meltedData <- melt(cbind.data.frame("Probe_ID"=rownames(data_stand_MedianByTP), "Cluster"=classif$Cluster, data_stand_MedianByTP), id.vars=c("Probe_ID", "Cluster"), variable.name="TimePoint",na.rm=TRUE)
		...
		if(time_unit!=""){
			meltedData$lab <- paste(time_unit, meltedData$TimePoint, sep="")
			meltedStats$lab <- paste(time_unit, meltedStats$TimePoint, sep="")
		}else{
			meltedData$lab <- as.numeric(meltedData$TimePoint)
			meltedStats$lab <- as.numeric(meltedStats$TimePoint)
		}
		...
		plot_breaks <- as.numeric(levels(as.factor(meltedData$TimePoint)))
		if(time_unit!=""){
			plot_labels <- paste(time_unit, plot_breaks, sep="")
		}else{
		...
		p <- (ggplot(meltedData, aes_string(x="TimePoint", y="value"))
			  ...
			  + scale_x_continuous(breaks=plot_breaks, labels=plot_labels[order(plot_breaks)])
		)
