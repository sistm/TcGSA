...
plot1GS <- 
	function(expr, gmt, Subject_ID,TimePoint, geneset.name, 
		...
		if(showTrend){
			...
			if(!smooth){
				if(clustering && !pre_clustering){
					p <- (p + geom_line(data=meltedStats, aes_string(x="TimePoint", y="value", group="Cluster", linetype="Cluster"), size=3))
				}else if(!clustering && pre_clustering){
					p <- (p + geom_line(data=meltedStats, aes_string(x="TimePoint", y="value", group="Group", linetype="Group"), size=3))
				}
			}else{
				...
				if(length(unique(meltedStats$Cluster))==1 | length(unique(meltedStats$Group))==1){
					...
					if(!clustering && pre_clustering){
						p <- (p 
							  + geom_smooth(formula=y~poly(x,3), data=meltedStats, aes_string(x="TimePoint", y="value", group="Group", linetype="Group", size="3"), colour="black", se=FALSE, method="lm")
						)
					}else{
						p <- (p 
				...
				if(!clustering && pre_clustering){
					p <- (p + guides(size="none")
						  + scale_linetype_manual(name=paste("Group", capwords(trend.fun)), values=as.numeric(levels(meltedStats$Cluster))+1,#as.numeric(levels(meltedStats$Cluster))+1, 
