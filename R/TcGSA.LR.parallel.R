      ...
      if(is.null(group.var)){
        ...
        if(!separatePatients){
          if(length(levels(data_lm$probe))>1){
            ...
            if(func=="linear"){
              ...
              lmm_H1 <- lmer(expression ~ 1 + probe + t1 + t2 + t3 + (t1 + t2 + t3 | probe) + (1|Patient_ID:probe), REML=FALSE, data=data_lm)
            }else if(func=="splines"){
        ...
        if(length(levels(data_lm$probe))>1){
          ...
          if(func=="linear"){
            ...
            lmm_H1 <- lmer(expression ~ 1 + Group + t1 + t1:Group + t2 + t2:Group + t3 + t3:Group + (1|Patient_ID), REML=FALSE, data=data_lm) 
          }else if(func=="splines"){
